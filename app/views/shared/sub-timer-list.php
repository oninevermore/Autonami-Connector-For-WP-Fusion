<?php if(is_array($model) && sizeof($model) > 0){?>
    <?php foreach($model as $item){?>
        <!--begin::Item-->
        <div data-id="<?=$item->id?>" class="d-flex align-items-center mb-9 rounded p-5 item-panel" style="background-color: <?=$item->bg_color?>">
            <!--begin::Icon-->
...
<!--                <button title="Duplicate Timer" data-toggle="tooltip" data-theme="dark" type="button" data-id="<?=$item->id?>" class="btn btn-icon btn-warning btn-circle btn-sm mr-2 duplicate"><i class="fab fa-creative-commons-share"></i></button>-->
                ...
                <input type="hidden" name="sub_timers[]" value="<?=$item->id?>" />
            </span>
            <!--end::Lable-->
        </div>
        <!--end::Item-->
    <?php }?>
<?php }else{?>
    <div class="alert alert-info text-center">There were no saved Sub Timer Sets yet. You can start adding new Sub Timer Sets now.</div>
<?php }?>